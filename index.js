...
exports = module.exports = function (options = {}) {
  ...
  return async function (ctx, next) {
    ...
    try {
      const stat = await fs.stat(ctx.browse.path);
      ctx.browse.stat = await fs.stat(ctx.browse.path);
      if (options.showStat) {
        ctx.browse.result = ctx.browse.stat;
      }
    } catch (e) {
...
exports.dirList = function (options = {}) {
  ...
  return async function (ctx, next) {
    if (!ctx.browse || ctx.browse.error || ctx.browse.result || !ctx.browse.path || !ctx.browse.stat || !ctx.browse.stat.isDirectory()) {
      return await next();
    }
    ...
    if (blacklist) {
      ctx.browse.result = ctx.browse.result.filter(file => !blacklist.includes(file.filename.toLowerCase()));
    }
    ...
    if (options.sort) {
      ctx.browse.result.sort((a, b) => {
        if (a.directory && !b.directory) {
          ...
          const a2 = a.filename.toLowerCase();
          const b2 = b.filename.toLowerCase();
...
exports.sendFile = function (options = {}) {
  const extensionsWhitelist = options.extensionsWhitelist && options.extensionsWhitelist.map(ext => ext.replace(/^\./, '').toLowerCase());
  const extensionsBlacklist = options.extensionsBlacklist && options.extensionsBlacklist.map(ext => ext.replace(/^\./, '').toLowerCase());
  return async function (ctx, next) {
    if (!ctx.browse || ctx.browse.error || ctx.browse.result || !ctx.browse.path || !ctx.browse.stat || !ctx.browse.stat.isFile()) {
      return await next();
    }
    if (extensionsWhitelist) {
